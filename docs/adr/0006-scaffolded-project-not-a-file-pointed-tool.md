...
There is no `speechdeck.config.*`. **Deck** facts stay in **Frontmatter**; project facts stay in `package.json` and `vite.config.ts`. Author modules and a custom **Theme** are ordinary specifiers: relative from the **Deck** file, bare through the project. Distribution is the JSR libraries plus an `init` create-package. A runtime binary that owns the build was rejected — that is the tool shape again. `dev` / `build` / `present` do not wrap Vite either; scaffolded scripts call Vite, and `init` is the only command (ADR 0007).
