...
`init()` never prompts. Clack is one client: on a TTY it asks directory (if omitted), starter (`skeleton` | `empty`), and theme (select of Harbour, Ink, Signal). Default is Harbour (`@speechdeck/themes/harbour`). No customize-gate — two questions do not need a third. Flags map 1:1 onto the options object. `--yes` / `-y` (and no TTY) skip prompts and apply defaults; they do not overwrite. Overwrite is `--existing overwrite`. `init` writes files only: no install, no network, no resume on partial write.
