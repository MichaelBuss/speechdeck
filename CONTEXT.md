...
- A **Slide** may have **Frontmatter** immediately after its `---` — bare `key: value` lines ended by a blank line, never a second `---` (a `---` there starts the next **Slide**, empty, and is a **Lint**). In v0 the keys are `layout` and `enter`. Empty **Frontmatter** is omitted. `theme`, `appearance`, and `motion` are **Deck**-only.
